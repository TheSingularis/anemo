...
- **[CHANGELOG.md](CHANGELOG.md) gets a new entry as part of every version bump, before tagging.** One `##` section per app, one `### X.Y.Z - YYYY-MM-DD` subsection per version, grouped `#### Added`/`Changed`/`Fixed`/`Removed` where more than one applies. The release workflow reads the matching section straight out of this file for the GitHub Release notes (see `Extract changelog section for this release` in `build-release.yml`) instead of GitHub's auto-generated commit list, and **fails the release build if the entry is missing** - so an untagged release always means CHANGELOG.md needs updating first, not the other way around. Never link to a bare `#anchor` inside a changelog entry, since release notes are copy-pasted out of this file's context - use the full `https://github.com/TheSingularis/anemo/blob/main/CHANGELOG.md#...` form instead.
- **Never add Claude/Anthropic as a commit co-author or PR contributor in this repo.** No `Co-Authored-By: Claude...` trailers, no "Generated with Claude Code" footers - this overrides the tool's own default attribution behavior for this repo specifically.
